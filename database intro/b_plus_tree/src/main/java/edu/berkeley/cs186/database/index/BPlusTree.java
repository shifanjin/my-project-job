...
//        throw new UnsupportedOperationException("TODO(hw2): implement");
        LeafNode lfn = root.get(transaction, key);
        Optional<RecordId> thekey = lfn.getKey(key);
        return thekey;
...
//        throw new UnsupportedOperationException("TODO(hw2): implement");
        ...
        Optional<Pair<DataBox, Integer>> thepair = root.put(transaction, key, rid);
        ...
        if (thepair.isPresent()) {
            ...
            DataBox thekey = thepair.get().getFirst();
            Page thepage = root.getPage();
            int pnum = thepage.getPageNum();
            int thechild = thepair.get().getSecond();


            newkeys.add(thekey);
            newchild.add(pnum);
            newchild.add(thechild);


            InnerNode newInnerNode = new InnerNode(metadata, newkeys, newchild, transaction);
            this.root = newInnerNode;
...
//        throw new UnsupportedOperationException("TODO(hw2): implement");
        ...
        if (scanAll(transaction).hasNext()) {
            throw new BPlusTreeException("Can only bulkload to an empty tree! The tree is not empty!");
        }


        while (data.hasNext()) {

            Optional<Pair<DataBox, Integer>> thepair = this.root.bulkLoad(transaction, data, fillFactor);
...
//        throw new UnsupportedOperationException("TODO(hw2): implement");
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private BaseTransaction trans;

        // Constructor 1
        public BPlusTreeIterator(BaseTransaction transaction) {
            trans = transaction;
            leafNode = root.getLeftmostLeaf(transaction);
            iter = leafNode.scanAll();
        }

        // Constructor 2 with specify key
        public BPlusTreeIterator(BaseTransaction transaction, DataBox key) {
            trans = transaction;
            leafNode = root.get(transaction, key);
            iter = leafNode.scanGreaterEqual(key);
        }
...
//            throw new UnsupportedOperationException("TODO(hw2): implement");
            ...
            boolean hasnextiter = iter.hasNext();
            boolean hasrightsibling = leafNode.getRightSibling(trans).isPresent();

            return hasnextiter || hasrightsibling;
...
//            throw new UnsupportedOperationException("TODO(hw2): implement");
            ...
            } else {    // IF no rid in the current node, move to it's right sibling
                leafNode = leafNode.getRightSibling(trans).get();
                iter = leafNode.scanAll();
                return iter.next();
            }
