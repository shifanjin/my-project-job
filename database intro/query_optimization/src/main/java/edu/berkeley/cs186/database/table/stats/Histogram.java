...
public class Histogram {
    ...
    public void buildHistogram(BaseTransaction transaction, Table table, int attribute) {
        ...
        RecordIterator riter = table.iterator(transaction);
        if (riter.hasNext() == false) {
            return;
        }

        //1. first calculate the min and the max values
        Record currecord = riter.next();

        float value = 0;
        minValue = quantization(currecord, attribute);
        maxValue = minValue;

        while (riter.hasNext()) {
            value = quantization(riter.next(), attribute);
            minValue = Math.min(value, minValue);
            maxValue = Math.max(value, maxValue);
        }

        //2. calculate the width of each bin
        this.width = (maxValue - minValue) / buckets.length;


        //3. create each bucket object
        for (int i  = 0; i < buckets.length; i++) {
            float lowerbound = i * width;
            float upperbound = lowerbound + width;
            buckets[i] = new Bucket<>(lowerbound, upperbound);
        }


        //4. populate the data using the increment(value) method
        riter = table.iterator(transaction);

        while (riter.hasNext()) {
            Record currec = riter.next();
            float v = quantization(currec, attribute);
            int bucketindex = bucketIndex(v);
            buckets[bucketindex].increment(v);
        }
...
//        throw new UnsupportedOperationException("TODO(hw4): implement");
    ...
    private float [] allEquality(float qvalue) {
        ...
        for (int i = 0; i < this.buckets.length - 1; i++) {
            ...
            if ((float)bucket.getStart() <= qvalue & (float) bucket.getEnd() > qvalue) {
                result[i] = (float)1/(float)bucket.getDistinctCount();
                return result;
            }
        }
        if (qvalue <= this.maxValue) {
            Bucket bucket = this.buckets[this.buckets.length-1];
            result[this.buckets.length-1] = (float)1/(float)bucket.getDistinctCount();
        }
        return result;
    ...
    private float [] allNotEquality(float qvalue) {
        ...
        for (int i = 0; i < this.buckets.length - 1; i++) {
            ...
            if ((float) bucket.getStart() <= qvalue & (float) bucket.getEnd() > qvalue) {
                ...
                result[i] = (float) 1 - (float) 1 / (float) bucket.getDistinctCount();
                return result;
            }
        }
        ...
        if (qvalue <= this.maxValue) {
            Bucket bucket = this.buckets[this.buckets.length-1];
            result[this.buckets.length - 1] = (float) 1 - (float) 1/ (float) bucket.getDistinctCount();
        }
        return result;
    ...
    private float [] allGreaterThan(float qvalue) {
        ...
        for (int i = 0; i < this.buckets.length - 1; i++) {
            ...
            if ((float)bucket.getStart() <= qvalue & (float) bucket.getEnd() > qvalue) {
                ...
                java.util.Arrays.fill(result, i+1, this.buckets.length, (float) 1);
                return result;
            }
        }
        ...
        if (qvalue <= this.maxValue) {
            Bucket bucket = this.buckets[this.buckets.length-1];
            result[this.buckets.length-1] = ((float) bucket.getEnd() - qvalue) / this.width;
        }
    ...
    private float [] allLessThan(float qvalue) {
        ...
        for (int i = 0; i < this.buckets.length-1; i++) {
            ...
            if ((float)bucket.getStart() <= qvalue & (float) bucket.getEnd() > qvalue) {
                ...
                result[i] = (qvalue - (float)bucket.getStart()) / this.width;
                java.util.Arrays.fill(result, i+1, this.buckets.length, (float)0);
                return result;
            }
        }
        if (qvalue <= this.maxValue) {
            Bucket bucket = this.buckets[this.buckets.length - 1];
            result[this.buckets.length - 1] = (qvalue - (float) bucket.getStart())/ this.width;
        }
        return result;
